...
@Component({
  ...
  imports: [CommonModule, FormsModule, Menu2Component, PiePaginaPrincipalComponent],
  ...
  marcarComoLeida(notif: Notificacion): void {
    if (!notif.read) {
      this.notificacionesService.markAsRead(notif.id).subscribe({
        ...
        error: (error) => {
          console.error(`Error al marcar notificación ${notif.id} como leída:`, error);
        }
      });
    }
  }
...
filtrarYBuscar(): void {
  ...
  this.notificaciones = this.notificacionesOriginales.filter(notif => {
    ...
    const coincideTipo = this.tipoFiltro === 'TODAS' || notif.type === this.tipoFiltro;
